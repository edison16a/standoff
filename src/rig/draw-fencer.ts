...
export function drawFencer(brush: Brush, skin: Skin, joints: Joints, frame: FencerFrame): void {
  ...
  drawArm(brush, skin, joints.backShoulder, joints.backElbow, joints.backHand);
  ...
  at(ctx, joints.head, joints.headAngle - Math.PI / 2, () => drawHead(brush, skin));
...
function drawSwordArm(brush: Brush, skin: Skin, joints: Joints, parrying: boolean): void {
  ...
  drawArm(brush, skin, joints.shoulder, joints.elbow, joints.hand);
  ...
  drawGlove(brush, skin.tones.glove, joints.hand, upper * 0.95);
}
