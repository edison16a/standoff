...
* `fighter-view.ts`: turns the fighter to face along the stage with a small turn toward the camera, picks the pose, glows when hit (brighter for bigger hits but never full white, and dimmer when one hit lands on several fighters at once, see `hit-flash.ts`), shivers in hit stop, blinks while invincible and squashes on landings. `fighter-trails.ts` lays a ribbon behind the fist, foot, blade tip or staff gem while a move is live, a streak and smoke behind a fighter flung by a big hit, and a mark where each big hitbox comes out. `fighter-extras.ts` draws the shadow, the colour marker over the head, the shield bubble, dizzy stars, the respawn platform and the ult ready ring.
...
The loop starts 14.6 seconds into the match, so its film holds four KOs: a karate punch flings the samurai out, the bear's Earthquake sends the karate after, and the samurai's Thousand Cuts takes out the mage and the bear together. The poster and the icon hold the moment the bear's Charge lands on the karate, with the samurai leaping on the left platform and the mage on the right. For stills the match is run ahead without drawing, then the pinned camera cuts in and one frame is drawn.
