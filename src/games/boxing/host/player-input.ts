import type { Body, MoveState, Side } from "@/games/kit/camera";
import { NO_DEFENSE, type DefenseInput } from "../engine/types";

/**
 * A player's defence from the camera, all from the waist up. Both gloves
 * up in front of the face is a block. The head dipping under its line is
 * a duck, and the head moving off to the side is a slip (`SlipReader`).
 ...
 */
export function defenseFrom(moves: MoveState | null, body: Body | null, slip: Side = moves?.lean ?? 0): DefenseInput {
