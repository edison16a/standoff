...
2. On the phone, set up in three steps, one page each:
   ...
   * **Gun:** your own BB gun turns on a stand in 3D. Pick a finish: Walnut, Cherry, Midnight, Birch, Mint or Showman. Drag to turn it.
   * **Ready:** tap Ready.
...
Each player has a pump BB gun along the bottom of the screen, front half in view like the cover, turning to follow their aim. It has a laser in their colour and a glowing dot where a shot would land. Firing kicks it back with a puff from the muzzle, a short BB streak flies to the target, and the pump is worked for the next shot.
...
* After a host reload the phones are asked to resend their setup, so nobody has to start over.
* The best 8 scores for each round length are kept in this computer's browser (localStorage) only. Nothing is sent to a server.
