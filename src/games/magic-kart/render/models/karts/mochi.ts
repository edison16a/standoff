...
import { ball, box, cyl, lathe, merge, mirrorX, paint, profile, rod } from "../geo";
...
export function buildMochi(): KartDesign {
  const body: THREE.BufferGeometry[] = [
    ...
    paint(profile([[1.3, 0.3], [1.32, 0.62], [1.05, 0.86], [0.3, 0.92], [0.1, 0.9], [0.1, 0.3]], 1.4, 0.18, true), PINK),
    paint(profile([[-0.5, 0.3], [-0.5, 0.95], [-1.0, 1.0], [-1.35, 0.78], [-1.38, 0.3]], 1.45, 0.18, true), PINK),
  ...
  const sides: THREE.BufferGeometry[] = [
    ...
    paint(ball(0.46, 18, 10, ), PINK, { at: [0.72, 0.55, 0.85], scale: [0.7, 0.7, 1] }),
    paint(ball(0.5, 18, 10), PINK, { at: [0.72, 0.58, -0.8], scale: [0.7, 0.72, 1] }),
    ...
    paint(cyl(0.13, 0.13, 0.08, 16), CREAM, { at: [0.42, 0.68, 1.33], rot: [Math.PI / 2 - 0.4, 0, 0] }),
    paint(cyl(0.08, 0.1, 0.3, 12), "#bfb6c7", { at: [0.4, 0.42, -1.5], rot: [Math.PI / 2, 0, 0] }),
  ];
  const glow = merge(mirrorX([
    paint(ball(0.1, 12, 8), "#fffbe8", { at: [0.42, 0.7, 1.38], scale: [1, 1, 0.5] }),
