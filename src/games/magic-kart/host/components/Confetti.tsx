...
/**
 * Confetti over the results, fired up from both bottom corners and
 * floating down. Plain 2D canvas, drawn only while the results are up.
 */
export function Confetti() {
  ...
  useEffect(() => {
    ...
    const fire = (fromLeft: boolean) => {
      for (let i = 0; i < COUNT / 2; i++) {
        const angle = ((60 + Math.random() * 28) * Math.PI) / 180;
  ...
  return <canvas ref={canvasRef} className="mk-confetti" aria-hidden="true" />;
}
