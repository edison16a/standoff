...
describe("the team lobby", () => {
  it("puts new players on the smaller team and fills the rest with computers", () => {
    ...
    readyPlayer(lobby, 1, "curry");
    readyPlayer(lobby, 2, "lebron");
    readyPlayer(lobby, 3, "jokic");
  ...
  it("gives each star to one player only", () => {
    ...
    lobby.connect(1);
    lobby.connect(2);
    expect(lobby.pick(1, "wemby")).toBe(true);
    expect(lobby.pick(2, "wemby")).toBe(false);
    expect(lobby.taken(2)).toEqual(["wemby"]);
  });

  it("keeps three a side when the host moves a player onto a full team", () => {
    const lobby = new Lobby();
    const stars = ["curry", "lebron", "durant", "giannis", "jokic", "doncic"] as const;
  ...
  it("remembers a dropped player's team and pick", () => {
    const lobby = new Lobby();
    readyPlayer(lobby, 1, "tatum");
    lobby.setTeam(1, 1);
    lobby.disconnect(1);
    lobby.connect(1);
    expect(lobby.seats.get(1)!.team).toBe(1);
    expect(lobby.seats.get(1)!.pick).toBe("tatum");
  });
});
