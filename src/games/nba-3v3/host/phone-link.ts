...
export class PhoneLink {
  ...
  sendState(seat: number, state: PhoneState, nowMs: number): void {
    ...
    const c = state.court;
    const urgent = `${state.phase}|${state.pick}|${state.ready}|${state.team}|${state.playing}|${c?.hasBall}|${c?.attacking}|${c?.mustClear}|${c?.canSteal}|${c?.countdown}|${c?.score.join()}|${c?.onFire}|${c?.checking}|${c?.freeThrow?.n}|${c?.freeThrow?.ready}`;
