...
export class Commentary {
  ...
  private play(e: Score, m: Match, name: string): string {
    if (e.kind === "dunk") {
      const style = CHARACTERS[m.athletes[e.id]!.character].dunkName.toLowerCase();
  ...
  private extra(e: Score, m: Match, name: string): string | null {
    ...
    if (e.streak === 4) return `${name} is on fire!`;
    if (e.streak === 3) return `${name} is heating up!`;
    if (mine === other) return `Tie game, ${mine} all.`;
    if (mine > other && mine - e.points < other) return `${team} take the lead!`;
    if (e.streak === 2 && this.nth("again") % 2 === 0) return this.pick("again-line", [`${name} again!`, `Back to back for ${name}!`]);
