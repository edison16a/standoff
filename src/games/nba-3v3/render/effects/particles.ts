...
/**
 * A pool of glowing points: sparks off the rim, sweat off a dunker,
 * flames round a ball on fire, the bursts of fireworks. One draw for all
 * of them. Each fades and shrinks over its life.
 */
export class Particles {
  ...
  update(dt: number): void {
    ...
    for (let i = 0; i < this.max; i++) {
      ...
      pos[i * 3 + 1] = Math.max(0.02, (pos[i * 3 + 1] ?? 0) + vy * dt);
