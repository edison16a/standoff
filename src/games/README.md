...
It opens `/showcase/<game-id>` (development only), writes the icon and poster into `media/` and the clip into `public/games/<game-id>/`. Wire them into `info.ts` as `media: { icon, poster, video: { webm, mp4 } }`.
