...
export class LoopMusic {
  ...
  /** A duck stage of its own, since the stingers share the music bus and must not dip with it. */
  ...
  constructor(private readonly engine: AudioEngine, cutoffHz: number) {
    ...
    this.dipGain = engine.ctx.createGain();
    this.tone.connect(this.dipGain).connect(engine.bus("music"));
  }
  ...
  /** Dips the loop to `amount` of its level, then eases back after `holdS`. */
  dip(amount: number, holdS: number): void {
    const gain = this.dipGain.gain;
    gain.cancelScheduledValues(this.engine.now);
    gain.setTargetAtTime(amount, this.engine.now, 0.08);
    gain.setTargetAtTime(1, this.engine.now + holdS, 0.6);
  }
