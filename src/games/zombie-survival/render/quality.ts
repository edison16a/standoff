 ...
 * their models can be looked at. Add &zpose=attack or &zpose=dead.
 */
export function gallery(): { kinds: string[]; pose: string } | null {
  if (typeof window === "undefined") return null;
  const params = new URLSearchParams(window.location.search);
  const kinds = params.get("zgallery");
  return kinds ? { kinds: kinds.split(","), pose: params.get("zpose") ?? "walk" } : null;
}
