...
import { cabTexture, carSideTexture, LIVERIES } from "../art/train-art";
...
function textured(key: string, texture: () => THREE.Texture): THREE.Material {
  let material = materials.get(key);
  if (!material) {
    material = new THREE.MeshStandardMaterial({ map: texture(), roughness: 0.38, metalness: 0.08 });
...
function buildCar(livery: number, role: CarRole, graffiti: number | null, lit: boolean): THREE.Group {
  ...
  b.box(W - 0.14, 0.34, L - 0.1, { color: 0xe4e8ee, finish: "satin" }, [0, BODY_TOP + 0.12, mid], undefined, 0.14);
  for (const x of [-0.72, 0.72]) b.box(0.3, 0.1, L - 1.6, { color: 0xb8c0ca, finish: "metal" }, [x, TRAIN.height - 0.02, mid], undefined, 0.04);
  ...
  const side = textured(`side-${livery}-${graffiti}`, () => carSideTexture(livery, graffiti));
