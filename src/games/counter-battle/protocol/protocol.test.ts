...
describe("the messages", () => {
  it("accept what the host and phones send", () => {
    ...
    for (const kind of ["reload", "hello", "again"]) expect(phoneMessageSchema.parse({ kind }).kind).toBe(kind);
  ...
  it("never use a kind the platform or the aim kit keeps for itself", () => {
    for (const kind of ["state", "buzz", "gun", "ready", "trigger", "reload", "hello", "again"]) {
