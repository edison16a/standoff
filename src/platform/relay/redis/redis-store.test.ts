...
describe.skipIf(!url)("RedisStore", () => {
  ...
  it("keeps a room whose host has left for about a minute, not hours", async () => {
    ...
    await store.create(newRoom({ code: "TTLA", hostToken: "token", joinUrl: "https://x/join/TTLA", hostConn: "conn", game: "blade-clash", seats: 2 }));
  ...
  it("refuses to write once another instance has taken the lock", async () => {
    const store = new RedisStore(redis);
    await store.create(newRoom({ code: "LOCK", hostToken: "token", joinUrl: "https://x/join/LOCK", hostConn: "first", game: "blade-clash", seats: 2 }));
    ...
    const change = () => {
      ...
      return { room: { ...newRoom({ code: "LOCK", hostToken: "token", joinUrl: "https://x/join/LOCK", hostConn: "stale", game: "blade-clash", seats: 2 }) }, result: true };
    };
